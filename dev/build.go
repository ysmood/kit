...
func genReadme() {
	fexmaple := E1(ReadStringFile("gokit_test.go")).(string)

	fset := token.NewFileSet()
	fast := E1(parser.ParseFile(fset, "gokit_test.go", fexmaple, parser.ParseComments)).(*ast.File)

	guardHelp := Exec("go", "run", "./cmd/guard", "--help").MustString()
	...
	f := E1(ReadStringFile("readme.tpl.md")).(string)
